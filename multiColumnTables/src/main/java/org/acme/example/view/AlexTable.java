...
//this class is not copyright; it was merely inspired by oracle's tutorial
public class AlexTable extends JPanel {
    private final static Logger LOGGER = LoggerFactory.getLogger(AlexTable.class);
	...
	TableModelTest model;
	JScrollPane scroll;
	
	public AlexTable(AbstractComponent component) {
		super(new GridLayout(1,0));
		model = new TableModelTest(component);
		table = new JTable(model);
		table.setAutoCreateRowSorter(true);
		table.setPreferredScrollableViewportSize(new Dimension(400,275)); //+++ TODO
		table.setFillsViewportHeight(true);
		scroll = new JScrollPane(table);
		add(scroll);
	}
	
	//the table model used to be a inner class here but was taken out. 
	...
	public void updateColumnsFromModel(int[] columnWidths) {
		TableModelTest model = TableModelTest.class.cast(table.getModel());
